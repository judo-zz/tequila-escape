...
この仕様書は `card-mode.html` / `card-mode.js` / `card-mode.css` の現行実装に基づく。旧 `GAME_GUIDE.md` や `HANDOFF.md` には酔い・疑惑・空気・MOOD ゲージ制の説明が一部残っているが、現行 CARD MODE は HP/P 制で実装されているため、本書ではコードを正とする。
...
プレイヤーと CPU みるくが、各ターン同時に 1 枚ずつカードを出す。最大 10 ターン。双方は HP と P を持ち、HP が 0 になった側が敗北する。10ターン終了時は HP、次に P の順で判定する。
...
`gauges.drunk/sus/tens/mood` は表示 ID 互換のため残っており、現行ではそれぞれ「あなた HP」「あなた P」「みるく HP」「みるく P」を表す。
...
以下は上限/下限にかからない通常時の差分。`あなた` と `みるく` はそれぞれ HP/P 差分を表す。

| あなた \\ みるく | 乾杯する | 飲んだフリ | 見張る | チェイサー |
|---|---|---|---|---|
| 乾杯する | あなた HP-1 P+1 / みるく HP-1 P+1 | あなた HP-1 P+1 / みるく P-1 | あなた HP-1 P+1 / みるく P-3 | あなた HP-1 P+1 / みるく HP+2 P-2 |
| 飲んだフリ | あなた P-1 / みるく HP-1 P+1 | あなた P-1 / みるく P-1 | あなた HP-2 P-1 / みるく P-2 | あなた P-1 / みるく HP+2 P-2 |
| 見張る | あなた P-3 / みるく HP-1 P+1 | あなた P-2 / みるく HP-2 P-1 | あなた P-3 / みるく P-3 | あなた P-3 / みるく HP+2 P-2 |
| チェイサー | あなた HP+2 P-2 / みるく HP-1 P+1 | あなた HP+2 P-2 / みるく P-1 | あなた HP+2 P-2 / みるく P-3 | あなた HP+2 P-2 / みるく HP+2 P-2 |
...
- 全合法手に初期重み 1 を付与する。
- みるく P が 1 以下なら `toast` を強める。
- みるく HP が 3 以下なら `fake` を強める。
- プレイヤー P が 1 以上なら `watch` を少し強め、2 以上、4 以上で段階的に警戒を強める。
...
| `TURN_START` | ゲーム画面表示、HUD/履歴/ACT 更新、次状態へ |
...
`maybeShowActBanner()` はターン 4、8、10 のバナー用関数として残っているが、現行 `enterTurnStart()` では `bannerDelay = 0` で直接 TELL_PHASE に進む。
...
5. 10ターン判定では HP 高い側が勝ち、HP 同値なら P 高い側が勝ち、P も同値なら draw
...
| `point_judge_win` | ショット銀行 | 10ターン HP 同点から P 差勝ち |
...
`preloadImage()` は一度読んだ画像を `imageCache` に記録し、同じ画像の二重デコードを避ける。`warmGameImagesSoon()` は対戦開始時に伏せカード、通常表情、乾杯カードだけを先に読み、それ以外は `requestIdleCallback` で少しずつ温める。カード選択直前には、選んだカードとみるくの予定カードだけを追加で先読みする。
...
- `#sfx-text`
- `#impact-layer`, `#commit-card`, `#reaction-burst`, `#match-cutin`, `#bust-cutin`, `#act-banner`
