...
const TELL_HOLD_MS        = 560;
const COMMIT_MS           = 400;
const REVEAL_PRE_FLIP_MS  = 600;
const CARD_GUIDE_HOLD_MS  = 420;
...
const LIFE_MAX            = 5;
const POINT_MAX           = 8;
const POINT_LABEL         = 'ノリ';
const CHASER_HEAL         = 2;
const CARD_COSTS          = { toast:0, fake:1, watch:2, chaser:3 };
const SUSPICION_MAX       = 4;
const TENSION_MAX         = 5;
const MILK_MOOD_MIN       = -3;
const MILK_MOOD_MAX       = 3;

// ── Card definitions ──
const CARDS = {
  toast:  { id:'toast',  icon:'🥂', label:'乾杯する',   hint:'HP-1/ノリ+1', milkLabel:'乾杯する',   guide:'HPを1払ってノリを1作る。ノリ満タンでも飲むとHPだけ減る。' },
  fake:   { id:'fake',   icon:'🎭', label:'飲んだフリ', hint:'ノリ1/守る',   milkLabel:'飲んだフリ', guide:'ノリ1でHPを守る。みるくが乾杯ならノリ+1、見張られると大ダメージ。' },
  watch:  { id:'watch',  icon:'👁', label:'見張る',     hint:'ノリ2/見抜く', milkLabel:'見張る',     guide:'ノリ2で相手のフェイクを読む。刺さると相手HP大ダメージ、自分ノリ+1。' },
  chaser: { id:'chaser', icon:'💧', label:'チェイサー', hint:'ノリ3/HP+2',   milkLabel:'チェイサー', guide:'ノリ3でHPを2回復。満タン時は使えない。終盤の保険。' },
};
...
const CHAR_IMAGES = {
  ...
  deisui:  'assets/optimized/deisui.webp',   // 保留: 将来の重酔い演出用にも使える素材
  kibishi: 'assets/optimized/kibishi.webp',  // 厳しい
  haiboku: 'assets/optimized/haiboku.webp',  // 敗北
  tellGlass:   'assets/optimized/milk_tell_glass.webp',
  tellExcite:  'assets/optimized/milk_tell_excite.webp',
  tellBored:   'assets/optimized/milk_tell_bored.webp',
  tellPlayful: 'assets/optimized/milk_tell_playful.webp',
};

const CUTIN_IMAGES = {
  'watch-fake':  'assets/optimized/milk_cutin_watch_success.webp',
  'fake-watch':  'assets/optimized/milk_cutin_busted.webp',
  'fake-toast':  'assets/optimized/milk_cutin_fake_success.webp',
  'toast-toast': 'assets/optimized/milk_cutin_toast.webp',
};

const RESULT_IMAGES = {
  win:     'assets/optimized/milk_result_win.webp',
  draw:    'assets/optimized/milk_result_draw.webp',
  special: 'assets/optimized/milk_result_special.webp',
  true:    'assets/optimized/milk_result_true.webp',
};
...
const MATCH_EFFECTS = {
  'toast-toast':   { tone:'toast',  kicker:'乾杯成立', title:'カンッ！', word:'CHEERS', copy:'夜が一段、熱くなる。' },
  'toast-fake':    { tone:'toast',  kicker:'片側だけ本気', title:'飲み切った', copy:'HPを払って、次の一手のノリを作る。' },
  ...
  'fake-toast':    { tone:'fake',   kicker:'潜伏成功', title:'逃げた！', word:'FAKE', copy:'飲んだ顔だけ置いて、場を抜ける。' },
  'fake-fake':     { tone:'fake',   kicker:'同時に煙幕', title:'二人ともごまかした', copy:'笑顔の裏で読み合いが濃くなる。' },
  'fake-watch':    { tone:'bust',   kicker:'カウンター被弾', title:'大バレ！', word:'BUSTED', copy:'そのフリ、みるくには見えていた。' },
  ...
  'watch-fake':    { tone:'counter',kicker:'読み勝ち', title:'見抜いた！', word:'READ', copy:'フリの瞬間を、視線で撃ち抜いた。' },
...
const SPOTLIGHT_MATCHES = new Set(['watch-fake', 'fake-watch', 'fake-toast', 'toast-toast']);
const LIMIT_EFFECT = {
  tone: 'limit',
  kicker: 'LIMIT CALL',
  title: '勝負する！',
...
const LIMIT_CARD_GUIDES = {
  toast: '限界中: 乾杯でみるくHP-1追加',
  fake: '限界中: 見張られなければノリ+1追加',
  watch: '限界中: フェイクを刺すとHP-1追撃',
  chaser: '限界中: チェイサー回復+1',
};
const MILK_READ_HINTS = {
  normal: '通常: バランス型',
  doubt: '疑い: 見張り注意',
  warm: 'ご機嫌: 予告やや素直',
  salt: '塩対応: 予告を疑って',
  guard: '守り: チェイサー警戒',
  pressure: '本気: 攻めと見張り注意',
  last: 'ラスト: ほぼ罠',
};
...
const FIELD_EVENTS = [
  { id:'call',      kicker:'場の空気', title:'店内コール',     copy:'両者乾杯でノリ+1',       tone:'hot' },
  { id:'pressure',  kicker:'場の空気', title:'キャストの圧',   copy:'フェイク成功でノリ+1',   tone:'pink' },
  { id:'manager',   kicker:'場の空気', title:'店長巡回中',     copy:'フリ失敗の被弾+1',       tone:'sharp' },
  { id:'water',     kicker:'場の空気', title:'水ありますよ',   copy:'チェイサー回復+1',       tone:'cool' },
  { id:'hot_table', kicker:'場の空気', title:'卓が熱い',       copy:'見張り成功の被弾+1',     tone:'hot' },
  { id:'quiet',     kicker:'場の空気', title:'静かな卓',       copy:'違う手でテンション+1',   tone:'quiet' },
  { id:'mid_show',  kicker:'5TH',      title:'中間チェック',   copy:'フリと見張りが重い',     tone:'sharp' },
  { id:'last_order',kicker:'LAST',     title:'ラストオーダー', copy:'乾杯と見張りが重い',     tone:'last' },
];

const MILK_MODE_LABELS = {
  normal: '通常',
  doubt: '疑い',
  warm: 'ご機嫌',
  salt: '塩対応',
...
function currentTellHonestRate() {
  const base = tellHonestRate(state.turn);
  const moodBonus =
    state.milkMood >= 2 ? 0.08 :
    state.milkMood <= -2 ? -0.08 : 0;
  const suspicionPenalty = state.suspicion >= 3 ? -0.05 : 0;
  return clamp(base + moodBonus + suspicionPenalty, 0.35, 0.9);
}
function tellTrustLabel(rate) {
  if (rate >= 0.75) return '信頼度 高 / たまに嘘';
  if (rate >= 0.65) return '信頼度 中 / 嘘あり';
  if (rate >= 0.55) return '信頼度 低 / 疑って';
  return '信頼度 博打 / ほぼ罠';
}

function deriveMilkMode() {
  if (state.milk.life <= 2) return 'guard';
  if (state.suspicion >= 3) return 'doubt';
  if (state.milk.point >= 4) return 'pressure';
  if (state.milkMood >= 2) return 'warm';
  if (state.milkMood <= -2) return 'salt';
  if (state.turn >= 8) return 'last';
  return 'normal';
}
function visualMilkMode() {
  return state.turn >= 8 ? 'last' : deriveMilkMode();
}
...
function setCharFace(exprKey) {
  ...
  if (charEl) {
    charEl.dataset.face = faceMap[exprKey] || 'normal';
    charEl.dataset.milkMode = visualMilkMode();
  }
  const src = CHAR_IMAGES[exprKey];
  if (imgEl && src && imgEl.src !== new URL(src, document.baseURI).href) {
    const swapToken = ++faceSwapToken;
    const completeSwap = () => {
      if (swapToken !== faceSwapToken) return;
      imgEl.src = src;
      imgEl.classList.add('loaded');
      imgEl.classList.remove('face-current-out', 'face-swap');
      if (nextEl) {
        nextEl.hidden = true;
        nextEl.classList.remove('loaded', 'face-next-in');
    ...
    preloadImage(src).then(() => {
      ...
      if (nextEl) {
        ...
        nextEl.classList.remove('loaded', 'face-next-in');
        nextEl.src = src;
        if (nextEl.complete) nextEl.classList.add('loaded');
        void nextEl.offsetWidth;
        nextEl.classList.add('face-next-in');
        imgEl.classList.remove('face-current-out', 'face-swap');
        void imgEl.offsetWidth;
        imgEl.classList.add('face-current-out');
        faceSwapTimer = window.setTimeout(completeSwap, 520);
      } else {
        imgEl.src = src;
...
function faceFromGauges() {
  ...
  if (state.turn >= 8 && state.suspicion >= 2) return 'kibishi';
  if (p.life <= 2)       return 'utagai';
  if (state.suspicion >= 3) return 'kibishi';
  if (m.point >= 4)      return 'kibishi';
  if (m.point >= 3)      return 'fuman';
  if (state.milkMood >= 2) return 'horoyoi';
  if (state.milkMood <= -2) return 'fuman';
...
function addSuspicion(amount) {
  state.suspicion = clamp(state.suspicion + amount, 0, SUSPICION_MAX);
}

function addTension(amount) {
  state.tension = clamp(state.tension + amount, 0, TENSION_MAX);
}

function addMilkMood(amount) {
  state.milkMood = clamp(state.milkMood + amount, MILK_MOOD_MIN, MILK_MOOD_MAX);
}

function rollFieldEvent() {
  if (state.turn === 5) {
    return FIELD_EVENTS.find(e => e.id === 'mid_show') || null;
  }
  if (state.turn < 8 && Math.random() < 0.45) return null;
  if (state.turn >= 8 && state.turn < MAX_TURNS && Math.random() < 0.25) return null;
  let candidates = state.turn >= 8
    ? FIELD_EVENTS.filter(e => ['last_order', 'hot_table', 'manager', 'call', 'water'].includes(e.id))
    : FIELD_EVENTS.filter(e => e.id !== 'last_order');
  if (state.turn === MAX_TURNS) {
    candidates = FIELD_EVENTS.filter(e => e.id === 'last_order');
  }
  const previous = state.field?.id;
  if (candidates.length > 1) candidates = candidates.filter(e => e.id !== previous);
  return candidates[Math.floor(Math.random() * candidates.length)] || FIELD_EVENTS[0];
}
...
function pickMilkCard() {
  ...
  // みるくも内部ポイントを読んで動く。序盤は飲んで資源を作り、中盤以降は読み合いに寄る。
  ...
  if (actorCanPlay(state.milk, 'watch')) {
    ...
    weights.watch += state.suspicion >= 3 ? 1.25 : state.suspicion >= 2 ? 0.7 : state.suspicion >= 1 ? 0.25 : 0;
  }
  ...
  const mode = deriveMilkMode();
  if ((mode === 'doubt' || mode === 'salt' || mode === 'pressure') && actorCanPlay(state.milk, 'watch')) {
    weights.watch += 0.65;
  }
  if (mode === 'guard' && actorCanPlay(state.milk, 'chaser')) {
    weights.chaser += 0.9;
  }
  if (mode === 'warm') {
    weights.toast += 0.35;
    if (actorCanPlay(state.milk, 'fake')) weights.fake += 0.25;
  }
  if (state.counts.fake >= 2 && actorCanPlay(state.milk, 'watch')) {
    weights.watch += 0.45;
  }
  if (state.counts.chaser >= 2) {
    weights.toast += 0.35;
  }
  if (state.field?.id === 'pressure' && actorCanPlay(state.milk, 'fake')) {
    weights.fake += 0.6;
  }
  if (state.field?.id === 'water' && actorCanPlay(state.milk, 'chaser')) {
    weights.chaser += 0.55;
  }
  if (state.field?.id === 'hot_table' && actorCanPlay(state.milk, 'watch')) {
    weights.watch += 0.55;
  }
...
function rollTell() {
  ...
  const honestRate = currentTellHonestRate();
  const honest = Math.random() < honestRate;
  ...
  return { type, honest, willPlay, chaserSpecial, honestRate };
}
...
function enterTurnStart() {
  ...
  clearReveal();
  clearSfx();
  state.field = rollFieldEvent();
  state.limitCallActive = false;
  updateHUD();
  updateActBadge();
  updateTurnHistory();
  renderFieldEvent();
...
function enterTell() {
  ...
  if (state.tell.type === 'glass' && (state.milk.point >= 5 || state.suspicion >= SUSPICION_MAX)) {
    EXPR_OF.glass = 'kibishi';
  }
...
function refreshHandSlots() {
  CARD_IDS.forEach(id => {
    ...
    const blockedReason = isFullBlocked ? '満タン' : (state.player.point < cost ? 'ノリ不足' : '不可');
    if (count) count.textContent = id === 'toast' ? (isToastCapped ? 'MAX' : 'FREE') : `ノリ${cost}`;
    ...
    slot.setAttribute('aria-label',
      isToastCapped ? `${CARDS[id].label} ノリ満タン。使うとHPが1減るだけ` :
        isFullBlocked ? `${CARDS[id].label} HP満タン` :
        `${CARDS[id].label} コスト${cost}ノリ`
    );
    slot.title = CARDS[id].guide;
...
function updateHandSubtitle() {
  ...
  const memory = state.cpuPattern.sameStreak >= 2 ? ' / 同じ手を覚えられてる' : '';
  if (state.turn === 1 && state.history.length === 0) {
    el.textContent = 'HP0で負け / 10TはHP→ノリ判定';
  } else if (state.field?.id === 'mid_show') {
    el.textContent = `中間チェック / フリと見張りが重い${memory}`;
  } else if (state.field?.id === 'last_order') {
    el.textContent = 'ラストオーダー / 乾杯と見張りが重い';
  } else if (state.limitCallActive) {
    el.textContent = '限界コール中 / この一手が強くなる';
  } else if (state.player.point <= 0 && playable.length === 1) {
    el.textContent = 'まず乾杯でノリを作る';
  } else if (state.player.point === 1 && actorCanPlay(state.player, 'fake')) {
    el.textContent = `フェイク可 / 見張りに注意${memory}`;
    ...
    el.textContent = `見張りも選べる / 読み勝負${memory}`;
  } else if (actorCanPlay(state.player, 'chaser') && state.player.life < LIFE_MAX) {
    el.textContent = '守るか、ノリを貯めるか';
  } else {
    el.textContent = '1枚えらんで同時に出す';
  }
}

function renderFieldEvent() {
  const wrap = $('field-event');
  if (!wrap) return;
  if (!state.field) {
    wrap.hidden = true;
    return;
  }
  wrap.dataset.tone = state.field.tone || 'quiet';
  $('field-event-kicker').textContent = state.field.kicker;
  $('field-event-title').textContent = state.field.title;
  $('field-event-copy').textContent = state.field.copy;
  wrap.hidden = false;
}

function renderLimitButton() {
  const btn = $('limit-btn');
  const effect = $('limit-effect');
  if (!btn) return;
  const canUse = state.fsm === 'CARD_SELECT' && (state.tension >= TENSION_MAX || state.limitCallActive);
  btn.hidden = !canUse;
  btn.disabled = state.limitCallActive;
  btn.classList.toggle('active', state.limitCallActive);
  btn.textContent = state.limitCallActive ? '限界コール中' : '限界コール';
  if (effect) {
    const activeCardText = state.limitCallActive
      ? '強化中: カードごとの追加効果が乗る。空振りはHP-1'
      : 'テンションMAXで次の1枚を強化。空振りはHP-1';
    effect.textContent = activeCardText;
    effect.hidden = !canUse;
  }
}

function onLimitCall() {
  if (state.fsm !== 'CARD_SELECT') return;
  if (state.limitCallActive) return;
  if (state.tension < TENSION_MAX) {
    showToast(`テンション${TENSION_MAX}で使える`);
    return;
  }
  state.tension = 0;
  state.limitCallActive = true;
  showToast('限界コール！次の1枚を強化。空振りはHP-1');
  playStylishCutin(LIMIT_EFFECT, 'limit');
  updateHUD();
  refreshHandSlots();
  renderLimitButton();
  updateHandSubtitle();
...
function onCardTap(cardId) {
  ...
  if (!actorCanPlay(state.player, cardId)) {
    ...
    showToast(cardId === 'chaser' && state.player.life >= LIFE_MAX ? 'HP満タン！' : `ノリ${cost}必要`);
    refreshHandSlots();
    return;
  }
...
function applyLimitPreviewToCards() {
  CARD_IDS.forEach(id => {
    const slot = document.querySelector(`.card-slot[data-card="${id}"]`);
    if (!slot) return;
    const hint = slot.querySelector('.card-hint');
    if (hint && state.limitCallActive) {
      hint.textContent = LIMIT_CARD_GUIDES[id].replace('限界中: ', '');
    }
    slot.classList.toggle('limit-boosted', state.limitCallActive);
  });
}

function showCardGuide(cardId) {
  const card = CARDS[cardId];
  if (!card) return;
  showToast(`${card.label}: ${card.guide}`);
}

function bindCardGuide(slot, cardId) {
  let holdTimer = 0;
  let guided = false;
  const clearHold = () => {
  ...
  slot.addEventListener('pointerdown', () => {
    ...
    holdTimer = window.setTimeout(() => {
      guided = true;
      showCardGuide(cardId);
    }, CARD_GUIDE_HOLD_MS);
  });
  slot.addEventListener('pointerup', clearHold);
...
function applyPsychologyAfterTurn(playerCard, milkCard, outcome, notes) {
  ...
  if (playerCard === 'fake') {
    addSuspicion(1);
    if (previous?.p === 'fake') addSuspicion(1);
  }
  if (playerCard === 'fake' && milkCard === 'toast') {
    addTension(2);
    addMilkMood(-1);
  }
  if (outcome.playerCounter) {
    addTension(2);
    addSuspicion(-1);
    addMilkMood(-2);
  }
  if (outcome.milkCounter) {
    addTension(1);
    addMilkMood(1);
  }
  if (playerCard === 'chaser') {
    addTension(-1);
    if (milkCard === 'toast') addMilkMood(-1);
  }
  if (playerCard === 'chaser' && milkCard === 'chaser') {
    addMilkMood(2);
    notes.push('一緒に水を挟んで、みるくの機嫌が戻った');
  }
  if (playerCard === 'toast' && milkCard === 'toast') {
    addMilkMood(1);
  }
  if (previous && previous.p !== playerCard) {
    addTension(1);
    if (state.field?.id === 'quiet') {
      addTension(1);
      notes.push('静かな卓で手を変えた。テンション+1');
    }
  }
  if (state.player.life <= 1) {
    addTension(1);
  }
}
...
function resolveBattleTurn(playerCard, milkCard) {
  const before = {
    ...
    milkMood: state.milkMood,
  };
  const fieldId = state.field?.id;
  const toastDamage = 1 + (fieldId === 'last_order' ? 1 : 0);
  const chaserHeal = CHASER_HEAL + (fieldId === 'water' ? 1 : 0);
  const watchDamage = 2 + (fieldId === 'hot_table' || fieldId === 'last_order' || fieldId === 'mid_show' ? 1 : 0);
  const fakeCaughtExtraDamage = fieldId === 'manager' ? 1 : 0;
  const caughtDamage = watchDamage + fakeCaughtExtraDamage;
  ...
  if (playerCard === 'toast') {
    ...
    addLife(state.player, -toastDamage);
    addPoint(state.player, 1);
    notes.push(wasPointMax ? `あなたは飲んだがノリ満タン / HP-${toastDamage}` : `あなたは飲んでHP-${toastDamage} / ノリ+1`);
  }
  if (milkCard === 'toast') {
    const wasPointMax = before.milk.point >= POINT_MAX;
    addLife(state.milk, -toastDamage);
    addPoint(state.milk, 1);
    notes.push(wasPointMax ? `みるくは飲んだがノリ満タン / HP-${toastDamage}` : `みるくは飲んでHP-${toastDamage} / ノリ+1`);
  }

  if (fieldId === 'call' && playerCard === 'toast' && milkCard === 'toast') {
    addPoint(state.player, 1);
    addPoint(state.milk, 1);
    notes.push('店内コール！両者ノリ+1');
  }

  if (playerCard === 'chaser') {
    addLife(state.player, chaserHeal);
    notes.push(`あなたはチェイサーでHP+${chaserHeal}`);
  }
  if (milkCard === 'chaser') {
    addLife(state.milk, chaserHeal);
    notes.push(`みるくはチェイサーでHP+${chaserHeal}`);
  }
  ...
  if (playerCounter) {
    addLife(state.milk, -caughtDamage);
    addPoint(state.player, 1);
    notes.push(`見張り成功！みるくHP-${caughtDamage} / あなたノリ+1`);
  }
  if (milkCounter) {
    addLife(state.player, -caughtDamage);
    addPoint(state.milk, 1);
    notes.push(`みるくの見張り成功！あなたHP-${caughtDamage} / みるくノリ+1`);
  }

  // Fake vs toast: stayed sober while milk drank → point +1 (offset cost, reward for correct read)
  if (playerCard === 'fake' && milkCard === 'toast') {
    const fakeReward = 1 + (fieldId === 'pressure' || fieldId === 'mid_show' ? 1 : 0);
    addPoint(state.player, fakeReward);
    notes.push(`場の隙を突いた！ノリ+${fakeReward}`);
  }
  if (milkCard === 'fake' && playerCard === 'toast') {
    const fakeReward = 1 + (fieldId === 'pressure' || fieldId === 'mid_show' ? 1 : 0);
    addPoint(state.milk, fakeReward);
    notes.push(`みるくが場の隙を突いた。みるくノリ+${fakeReward}`);
  }

  if (state.limitCallActive) {
    if (playerCard === 'toast') {
      addLife(state.milk, -1);
      notes.push('限界コール！みるくにもHP-1');
    } else if (playerCard === 'fake' && milkCard !== 'watch') {
      addPoint(state.player, 1);
      notes.push('限界コールでフェイクが通った！ノリ+1');
    } else if (playerCard === 'watch' && milkCard === 'fake') {
      addLife(state.milk, -1);
      notes.push('限界コールで見張り追撃！みるくHP-1');
    } else if (playerCard === 'chaser') {
      addLife(state.player, 1);
      notes.push('限界コールで踏みとどまった！HP+1');
    } else {
      addLife(state.player, -1);
      notes.push('限界コール空振り……あなたHP-1');
    }
    state.limitCallActive = false;
  }

  applyPsychologyAfterTurn(playerCard, milkCard, {
    playerCounter,
    milkCounter,
  }, notes);
  ...
  const outcome = {
    ...
    suspicionDelta: state.suspicion - before.suspicion,
    tensionDelta: state.tension - before.tension,
    milkMoodDelta: state.milkMood - before.milkMood,
    playerCounter,
    milkCounter,
    notes,
    before,
...
function enterResolve() {
  ...
  const spotlightNote = outcome.notes.find(n =>
    /成功|大バレ|限界|コール|ラスト|的中/.test(n)
  );
  showSfx(spotlightNote || outcome.notes[0] || SFX_LABELS[sfxKey] || '');

  updateHUD();
  showResourceDeltaPops(outcome);
...
function fieldEventById(id) {
  return FIELD_EVENTS.find(e => e.id === id);
}
...
const ACHIEVEMENT_RULES = [
  { id:'ACH_LIFE_RULE_WIN', name:'初勝利', description:'ライフとノリを管理して勝った。', test:c=>c.isWin },
  { id:'ACH_MILK_KO', name:'飲ませ切り', description:'みるくのライフを0にした。', test:c=>c.endReason==='milk_life0' },
  { id:'ACH_POINT_WIN', name:'判定の支配者', description:'ノリ差で勝負を持っていった。', test:c=>c.endReason==='turn_limit'&&c.winner==='player'&&c.player.life===c.milk.life },
  ...
  { id:'ACH_SUS_WIN', name:'疑惑まみれの逃亡', description:'疑われながらも勝ち切った。', test:c=>c.isWin&&c.suspicion>=3 },
  { id:'ACH_LIMIT_CALL', name:'限界コール', description:'テンションを切って一手を強化した。', test:c=>c.history.some(h=>h.outcome?.notes?.some(n=>n.includes('限界コール'))) },
  { id:'ACH_LAST_ORDER', name:'ラストオーダーの住人', description:'ラストオーダーの重い一手を経験した。', test:c=>c.history.some(h=>h.field==='last_order') },
];
...
function buildSummaryTexts({ ending, isWin }) {
  const decisive = findDecisiveTurn();
  ...
  const objective = [
    `総評: ${review}`,
    `プレイ傾向: ${style}`,
    `最終スコア: あなた HP${state.player.life} / ノリ${state.player.point}　みるく HP${state.milk.life} / ノリ${state.milk.point}`,
    `心理状態: 疑惑${state.suspicion}/${SUSPICION_MAX} / テンション${state.tension}/${TENSION_MAX} / みるく${MILK_MODE_LABELS[deriveMilkMode()]}`,
    `総ターン: ${state.turn} / ${MAX_TURNS}`,
    `最多カード: ${CARDS[topCard.id].icon} ${CARDS[topCard.id].label} ×${topCard.count}`,
    `リソース推移: あなた最低HP ${state.peak.playerLowLife} / 最高ノリ ${state.peak.playerHighPoint}　みるく最低HP ${state.peak.milkLowLife} / 最高ノリ ${state.peak.milkHighPoint}`,
    decisive
  ...
  return { objective, monologue, cast, topCard, decisive };
}

function findDecisiveTurn() {
  if (!state.history.length) return null;
  return state.history.reduce((best, h) => {
    const o = h.outcome || {};
    const score =
      Math.abs(o.playerLifeDelta || 0) * 4 +
      Math.abs(o.milkLifeDelta || 0) * 4 +
      Math.abs(o.playerPointDelta || 0) +
      Math.abs(o.milkPointDelta || 0) +
      (o.playerCounter || o.milkCounter ? 8 : 0) +
      (h.field === 'last_order' || h.field === 'mid_show' ? 2 : 0);
    return !best || score >= best.score ? { ...h, score } : best;
  }, null);
}

function buildDecisionReason(summary) {
  const d = summary.decisive;
  const prefix = summary.winner === 'player'
    ? '勝因'
    : summary.winner === 'milk'
      ? '敗因'
      : '分岐点';
  if (!d) return `${prefix}: 最後まで決定打なし`;
  const key = `${d.p}-${d.m}`;
  const note = d.outcome?.notes?.find(n => /成功|大バレ|空振り|ラスト|中間|限界|HP-|ノリ/.test(n))
    || SFX_LABELS[key]
    || `${CARDS[d.p].label} vs ${CARDS[d.m].milkLabel}`;
  const field = fieldEventById(d.field);
  const fieldText = field ? ` / ${field.title}` : '';
  return `${prefix}: T${d.turn} ${note}${fieldText}`;
}
...
function buildResultReview({ ending, isWin, topCard, decisive }) {
  ...
  if (state.winner === 'player') {
    if (r === 'milk_life0') {
      return 'テキーラを飲むリスクをノリに変えて、最後はみるくのライフを削り切った。新ルールの勝ち筋がかなり綺麗に出ている。';
    }
    if (r === 'turn_limit' && state.player.life > state.milk.life) {
      return '最後までHPを残して逃げ切った。飲むタイミングと守るタイミングの切り替えが勝因。';
    }
    return 'ライフは並んだが、ノリ管理で上回った。飲んで得た資源を最後まで腐らせなかった勝ち方。';
  }
  if (state.winner === 'draw') {
    return '読み合いは完全に拮抗。ライフもノリも決め手にならず、次の一戦に持ち越し。';
  }
  if (r === 'player_life0') {
    return 'ライフが先に尽きた。ノリを貯めるために飲む判断は大事だが、回復かフェイクへ切り替える一手が遅れた。';
  }
  if (state.winner === 'milk') {
    if (r === 'turn_limit' && state.player.life < state.milk.life) {
      return '10ターン走り切ったが、最後のHP差で届かなかった。飲む一手をどこかでフェイクかチェイサーに変えたい。';
    }
    return 'ライフは並んだが、ノリ管理でみるくに上回られた。乾杯で稼いだノリを、最後の読み合いに変換しきれなかった。';
  }
...
function buildPlayStyleLine({ topCard }) {
  const styleMap = {
    toast: [
      '正面突破型。HPを払ってノリを作る判断が勝負。',
      '乾杯で資源を作るタイプ。攻めの説得力はある。',
      '迷ったら前に出るプレイ。ノリは貯まるが、HP管理が課題。',
    ],
    fake: [
      '煙幕型。HPを守れるが、見張られた瞬間に一気に崩れる。',
      '飲まずにターンを受け流すタイプ。読み負けだけは絶対に避けたい。',
      'ごまかしで勝負を伸ばすプレイ。ノリ1の使いどころが大事。',
    ],
    watch: [
      '読みに寄せた防御型。相手のフリには強いが、ノリ2の投資は重い。',
      '視線で勝つタイプ。決まると気持ちいいが、空振り時の損失が重い。',
      '一撃狙いのカウンター型。ノリを貯めてからが本番。',
    ],
    chaser: [
      '生存重視型。HP回復は強いが、ノリ3を吐く判断が重い。',
      '引き際を作るタイプ。使いすぎると攻めるノリが足りなくなる。',
...
function buildMilkLine({ ending, topCard, decisive }) {
  ...
  const pools = {
    player_life0: [
      'ライフ、先になくなっちゃったね。ノリを貯めるのは大事だけど、飲みすぎ注意だよ。',
    ...
    turn_limit_loss: [
      '最後まで座ってたのはえらい。でも、HPかノリのどっちかで少し届かなかったね。',
      '10ターン目まで行ったのに、最後の差でみるくの勝ち。惜しかったよ。',
      '負けたけど、崩れたわけじゃない。次はノリの使いどころで変わると思う。',
...
function buildSummary() {
  ...
  state.summary = {
    ...
    peak: { ...state.peak },
    decisive: texts.decisive || state.history[state.history.length - 1] || null,
    turn: state.turn,
    texts: { record: texts.objective, monologue: texts.monologue, cast: texts.cast },
  };
  state.summary.reason = buildDecisionReason(state.summary);
  state.summary.texts.record.splice(1, 0, state.summary.reason);
}
...
function enterResult() {
  ...
  $('result-reason').textContent = s.reason || '';
  ...
  if (preview) {
    ...
    const resultKey =
      ...
      state.winner === 'milk' ? 'win' :
      s.ending === 'special' ? 'special' :
      s.ending === 'true' ? 'true' :
      null;
    preview.classList.toggle('result-portrait-wrap', Boolean(resultKey));
    const imgSrc = resultKey ? RESULT_IMAGES[resultKey] : CARD_IMAGES.player[cardId];
    if (imgSrc) {
      const img = document.createElement('img');
      img.src = imgSrc;
      img.alt = resultKey ? '試合後のみるく' : CARDS[cardId].label;
      img.className = resultKey ? 'result-card-art result-milk-art' : 'result-card-art';
...
function setResultTab(tab) {
  ...
  if (tab === 'record') {
    const record = s.texts.record
      .map(l => `<div class="pov-line">${escHtml(l)}</div>`)
      .join('');
    pov.innerHTML = `${record}<div class="pov-section-title">ターン記録</div>${buildResultHistoryHtml()}`;
  } else {
    pov.innerHTML = [
      `<div class="pov-cast">${escHtml(s.texts.cast)}</div>`,
      `<div class="pov-section-title">あなたの内省</div>`,
      `<div class="pov-monologue">${escHtml(s.texts.monologue)}</div>`,
    ].join('');
  }
}
...
function buildResultHistoryHtml() {
  ...
  return state.history.map(h => {
    ...
    const field = fieldEventById(h.field);
    const pDelta = formatDelta(h.outcome?.playerLifeDelta, 'HP') + formatDelta(h.outcome?.playerPointDelta, POINT_LABEL);
    const mDelta = formatDelta(h.outcome?.milkLifeDelta, 'HP') + formatDelta(h.outcome?.milkPointDelta, POINT_LABEL);
    return [
      ...
      field ? `<small>${escHtml(field.title)}</small>` : '',
...
function buildResultPostText() {
  ...
  const lines = [
    ...
    s.reason,
    resultLine,
    `あなた HP${s.player.life} / ノリ${s.player.point}　みるく HP${s.milk.life} / ノリ${s.milk.point}`,
...
function updateMindStrip() {
  const suspicion = $('suspicion-chip');
  const tension = $('tension-chip');
  const milkMode = $('milk-mode-chip');
  const mode = deriveMilkMode();
  const visualMode = visualMilkMode();
  if (suspicion) {
    suspicion.textContent = `疑惑 ${state.suspicion}/${SUSPICION_MAX}`;
    suspicion.dataset.level = state.suspicion >= 3 ? 'high' : state.suspicion >= 2 ? 'mid' : 'low';
  }
  if (tension) {
    tension.textContent = state.tension >= TENSION_MAX ? 'テンション MAX' : `テンション ${state.tension}/${TENSION_MAX}`;
    tension.dataset.level = state.tension >= TENSION_MAX ? 'max' : state.tension >= 3 ? 'mid' : 'low';
  }
  if (milkMode) {
    const label = MILK_READ_HINTS[visualMode] || MILK_READ_HINTS.normal;
    const suffix = state.turn >= 8 && visualMode !== mode ? ` / ${MILK_MODE_LABELS[mode]}` : '';
    milkMode.textContent = `みるく ${label}${suffix}`;
    milkMode.title = label;
    milkMode.dataset.mode = visualMode;
  }
  const app = $('app');
  if (app) app.dataset.milkMode = visualMode;
  const charEl = $('character');
  if (charEl) charEl.dataset.milkMode = visualMode;
}
...
function updateRecentHistory() {
  ...
  last3.forEach(h => {
    ...
    const mDelta = h.outcome?.milkLifeDelta ?? 0;
    const swing = pDelta - mDelta;
    const cls = pDelta <= -2 ? 'rh-bust' : mDelta <= -2 ? 'rh-good' : pDelta < 0 ? 'rh-bad' : 'rh-neutral';
    const delta = `YOU${formatSigned(pDelta)} / MILK${formatSigned(mDelta)}`;
    const swingText = swing > 0 ? '優勢' : swing < 0 ? '苦しい' : '五分';
    row.innerHTML =
      `<span class="rh-turn">T${h.turn}</span>` +
      `<span class="rh-cards">${CARDS[h.p].icon}${CARDS[h.m].icon}</span>` +
      `<span class="rh-delta ${cls}">${delta}</span>` +
      `<span class="rh-swing">${swingText}</span>`;
    el.appendChild(row);
  });
}
...
function showResourceDeltaPops(outcome) {
  const deltas = [
    ['drunk-value', outcome.playerLifeDelta, 'HP'],
    ['sus-value', outcome.playerPointDelta, POINT_LABEL],
    ['tens-value', outcome.milkLifeDelta, 'HP'],
    ['mood-value', outcome.milkPointDelta, POINT_LABEL],
  ];
  deltas.forEach(([id, value, label], index) => {
    if (!value) return;
    const target = $(id);
    if (!target) return;
    const pop = document.createElement('span');
    pop.className = `delta-pop ${value > 0 ? 'delta-plus' : 'delta-minus'}`;
    pop.textContent = `${label}${formatSigned(value)}`;
    pop.style.setProperty('--delta-delay', `${index * 45}ms`);
    target.closest('.hud-stat')?.appendChild(pop);
    setTimeout(() => pop.remove(), 1050 + index * 45);
  });
}
...
function updateDangerState() {
  ...
  else if (state.tension >= TENSION_MAX) danger = 'tension';
  else if (state.suspicion >= 3 || state.milk.point >= 3) danger = 'sus';
...
function playMatchImpact(matchKey, reactionFace) {
  ...
  const app = $('app');
  const strongCutin = SPOTLIGHT_MATCHES.has(matchKey);
  if (effect && cutin && strongCutin) {
    playStylishCutin(effect, matchKey);
  } else if (cutin) {
    cutin.classList.remove('cutin-in');
  }
  if (burst && img && (strongCutin || reactionFace !== 'tuzyou')) {
    const generatedCutin = CUTIN_IMAGES[matchKey];
    img.src = generatedCutin || CHAR_IMAGES[reactionFace] || CHAR_IMAGES.tuzyou;
    burst.className = `reaction-burst face-${reactionFace} tone-${effect?.tone || 'neutral'} match-${matchKey}`;
    burst.classList.toggle('has-generated-cutin', Boolean(generatedCutin));
...
function playStylishCutin(effect, variant = '') {
  ...
  cutin.dataset.variant = variant;
  cutin.dataset.word = effect.word || effect.title;
  cutin.classList.remove('cutin-in');
...
function playBustCutin() {
  ...
  if (!cutin || !img) return;
  img.src = CUTIN_IMAGES['fake-watch'] || CHAR_IMAGES.kibishi;
...
function warmGameImagesSoon() {
  const critical = [
    CHAR_IMAGES.tuzyou,
    CHAR_IMAGES.tellGlass,
    CHAR_IMAGES.tellExcite,
    CHAR_IMAGES.tellBored,
    CHAR_IMAGES.tellPlayful,
...
function init() {
  ...
  CARD_IDS.forEach(id => {
    const slot = document.querySelector(`.card-slot[data-card="${id}"]`);
    if (slot) {
      bindCardGuide(slot, id);
      slot.addEventListener('click', () => onCardTap(id));
    }
  });
  $('limit-btn')?.addEventListener('click', onLimitCall);
